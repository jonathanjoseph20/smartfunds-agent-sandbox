...
describe('investigation report generation', () => {
  it('T-INV-RPT1 writes stable machine-readable and markdown reports', () => {
    ...
    createEvidenceStore({ artifactsRoot: path.join(tmpRoot, 'artifacts') }).persistEvidence('run-1', [{
      evidenceId: 'e1',
      investigationRunId: 'run-1',
      phaseId: 'gather',
      evidenceType: 'raw_observation',
      sourceArtifactPath: path.join(runRoot, 'gather-evidence.json'),
      sourceDatasetKey: 'protocol_tvl_timeseries',
      summary: 'gather artifact captured',
      payload: {},
      findingIds: ['liquidity_drop:Aave:12']
    }]);
    ...
    expect(fs.readFileSync(report.jsonPath, 'utf8')).toContain('"reportConfidence"');
