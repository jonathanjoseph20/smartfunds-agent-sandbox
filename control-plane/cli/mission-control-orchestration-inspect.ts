...
function toStableError(error: unknown): string {
  ...
  if (message === 'MISSION_CONTROL_ORCHESTRATION_PLAN_NOT_FOUND') {
    return 'intervention_plan_not_found';
  }
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const result = createMissionControlOrchestrationInspection().inspectInterventionPlan(args);
    printJson(result);
    return 0;
  } catch (error) {
  ...
  main()
    ...
    .catch(() => {
      process.stdout.write(
        `${canonicalStringify({ error: 'MISSION_CONTROL_ORCHESTRATION_INSPECT_UNEXPECTED_RUNTIME_ERROR' })}\n`,
      );
      process.exit(2);
    });
}