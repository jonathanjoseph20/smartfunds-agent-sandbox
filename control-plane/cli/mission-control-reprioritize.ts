...
import type { MissionPriorityLevel } from '../mission-control/mission-coordination.ts';
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  let priority: MissionPriorityLevel | null = null;
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--priority') {
      const value = argv[index + 1];
      if (!value) {
        throw new Error('MISSING_ARGUMENT: --priority');
      }
      priority = value as MissionPriorityLevel;
      index += 1;
      continue;
    }
  ...
  return { missionRunId, requestedBy, priority, reasonTokens };
}
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const result = createMissionCoordination().reprioritizeMission(args);
    printJson(result);
    return 0;
  } catch (error) {
