...
import { createEngineeringPlanInspection } from '../engineering/engineering-plan-inspection.ts';
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const inspection = createEngineeringPlanInspection();
    printJson(inspection.inspectEngineeringPlan(args.planId));
    return 0;
  } catch (error) {
