...
import { createRuntimeEnvelopeHistoryStore } from '../../runtime-envelope/runtime-envelope-history-store.ts';
import { createRuntimeEnvelopeMaterializer } from '../../runtime-envelope/runtime-envelope-materializer.ts';
import { createRuntimeEnvelopeProjection } from '../../runtime-envelope/runtime-envelope-projection.ts';

const tmpRoot = path.join('control-plane', '__tests__', 'tmp-runtime-envelope-materializer');
...
describe('runtime envelope materializer', () => {
  it('T-MRE-M1 writes deterministic artifact set and preserves disabled runtime semantics', () => {
    ...
    const historyStore = createRuntimeEnvelopeHistoryStore({ artifactsRoot: fixtures.runtimeEnvelopeArtifactsRoot });
    const projection = createRuntimeEnvelopeProjection({
      historyStore,
      missionDefinitionsDir: fixtures.missionDefinitionsDir,
      missionInstancesDir: fixtures.missionInstancesDir,
      teamDefinitionsDir: fixtures.teamDefinitionsDir,
    ...
    const listed = projection.projectAll();
    const runtimeEnvelopeId = listed[0]?.runtimeEnvelopeId;
    if (!runtimeEnvelopeId) {
      throw new Error('RUNTIME_ENVELOPE_NOT_FOUND');
    }

    const materializer = createRuntimeEnvelopeMaterializer({
      projection,
      historyStore,
      runtimeEnvelopeArtifactsRoot: fixtures.runtimeEnvelopeArtifactsRoot,
    });

    const first = materializer.materializeOne({ runtimeEnvelopeId });
    const firstSnapshot = {
      ...
      payload: fs.readFileSync(first.payloadPath, 'utf8'),
      capabilities: fs.readFileSync(first.capabilitiesPath, 'utf8'),
    };

    const second = materializer.materializeOne({ runtimeEnvelopeId });
    const secondSnapshot = {
      ...
      payload: fs.readFileSync(second.payloadPath, 'utf8'),
      capabilities: fs.readFileSync(second.capabilitiesPath, 'utf8'),
    };

    expect(secondSnapshot).toEqual(firstSnapshot);
    expect(firstSnapshot.markdown).toContain('# Mission Runtime Envelope Report');
    ...
    const payload = JSON.parse(firstSnapshot.payload) as { missionSummary: string; deliverableScope: string[] };
    expect(payload.missionSummary.length).toBeGreaterThan(0);
    expect(Array.isArray(payload.deliverableScope)).toBe(true);
  });
});
