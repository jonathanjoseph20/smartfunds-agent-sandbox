...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    inspection.inspectIntelligenceSet(args);
    printJson(inspection.inspectIntelligenceHistory(args));
    return 0;
  } catch (error) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().then((code) => {
    ...
    process.stdout.write(`${canonicalStringify({ error: 'CROSS_PORTFOLIO_HISTORY_UNEXPECTED_RUNTIME_ERROR' })}\n`);
    process.exit(2);
  });
}
