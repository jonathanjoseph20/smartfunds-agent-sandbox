...
function toStableError(error: unknown): string {
  ...
  if (message === 'CROSS_PORTFOLIO_INTELLIGENCE_SET_NOT_FOUND') {
    return 'intelligence_set_not_found';
  }
  return message;
}
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    printJson(inspection.inspectIntelligenceSet(args));
    return 0;
  } catch (error) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().then((code) => {
    ...
    process.stdout.write(`${canonicalStringify({ error: 'CROSS_PORTFOLIO_INSPECT_UNEXPECTED_RUNTIME_ERROR' })}\n`);
    process.exit(2);
  });
}
