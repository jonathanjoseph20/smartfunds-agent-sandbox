...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    printJson(createMissionControlOrchestrationManager().materializeInterventionPlan(args));
    return 0;
  } catch (error) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().then((code) => {
    ...
    process.stdout.write(`${canonicalStringify({ error: 'MISSION_CONTROL_ORCHESTRATION_MATERIALIZE_UNEXPECTED_RUNTIME_ERROR' })}\n`);
    process.exit(2);
  });
}
