...
import { createProductSpecInspection } from '../products/product-spec-inspection.ts';
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const inspection = createProductSpecInspection();
    printJson(inspection.inspectProductSpec(args.specId));
    return 0;
  } catch (error) {
