...
function parseArgs(argv: string[]): { runtimeOutcomePropagationRecordId: string } {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg.startsWith('--recordId=')) {
      const value = arg.slice('--recordId='.length);
      if (!value) {
        throw new Error('MISSING_ARGUMENT: --record');
      }
      runtimeOutcomePropagationRecordId = value;
...
function toStableError(error: unknown): string {
  ...
  return message === 'RUNTIME_OUTCOME_PROPAGATION_RECORD_NOT_FOUND'
...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const result = createRuntimeOutcomePropagationInspection().inspectPropagationRecord(args);
    printJson(result);
    return 0;
  } catch (error) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().then((code) => {
    ...
    process.stdout.write(`${canonicalStringify({ error: 'MISSION_CONTROL_PROPAGATION_INSPECT_UNEXPECTED_RUNTIME_ERROR' })}\n`);
    process.exit(2);
  });
}
