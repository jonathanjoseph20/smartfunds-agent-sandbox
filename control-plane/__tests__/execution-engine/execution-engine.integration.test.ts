...
const tmpRoot = path.join('control-plane', '__tests__', 'tmp-execution-engine-integration');
...
describe('execution engine integration', () => {
  it('T-MEE-I1 deterministic bounded engine flow appends journal events and preserves upstream truth', () => {
    ...
    const executionAttemptProjection = createExecutionAttemptProjection({
      missionDefinitionsDir: fixtures.missionDefinitionsDir,
      missionInstancesDir: fixtures.missionInstancesDir,
      teamDefinitionsDir: fixtures.teamDefinitionsDir,
      compatibilityArtifactsRoot: fixtures.compatibilityArtifactsRoot,
    ...
    const attemptBeforeEngine = executionAttemptProjection.projectOne({ executionAttemptId: attempt.executionAttemptId });

    journalInspection.evaluateExecutionJournal({ executionAttemptId: attempt.executionAttemptId });

    const firstEval = engineInspection.evaluateEngineReadiness({ executionAttemptId: attempt.executionAttemptId });
    const secondEval = engineInspection.evaluateEngineReadiness({ executionAttemptId: attempt.executionAttemptId });
    expect(firstEval.executionEngineRunId).toBe(secondEval.executionEngineRunId);
    expect(firstEval.engineEligibilityState).toBe(secondEval.engineEligibilityState);

    const started = engineInspection.startEngineRun({ executionAttemptId: attempt.executionAttemptId });
    expect(started.engineState).toBe('running');

    const journalAfterStart = journalInspection.inspectExecutionJournal({ executionAttemptId: attempt.executionAttemptId });
    expect(journalAfterStart.events.some((event) => event.eventType === 'execution_started')).toBe(true);

    const completed = engineInspection.completeEngineRun({ executionAttemptId: attempt.executionAttemptId });
    expect(completed.engineState).toBe('completed');

    const journalAfterComplete = journalInspection.inspectExecutionJournal({ executionAttemptId: attempt.executionAttemptId });
    expect(journalAfterComplete.events.some((event) => event.eventType === 'execution_completed')).toBe(true);

    const materializedFirst = engineInspection.materializeEngineRun({ executionAttemptId: attempt.executionAttemptId });
    const materializedSecond = engineInspection.materializeEngineRun({ executionAttemptId: attempt.executionAttemptId });
    ...
    const firstSnapshot = {
      ...
      outputs: fs.readFileSync(materializedFirst.outputsPath, 'utf8'),
    };
    ...
    const secondSnapshot = {
      ...
      outputs: fs.readFileSync(materializedSecond.outputsPath, 'utf8'),
    };

    expect(secondSnapshot).toEqual(firstSnapshot);

    const attemptAfterEngine = executionAttemptProjection.projectOne({ executionAttemptId: attempt.executionAttemptId });
    expect(attemptAfterEngine).toEqual(attemptBeforeEngine);
