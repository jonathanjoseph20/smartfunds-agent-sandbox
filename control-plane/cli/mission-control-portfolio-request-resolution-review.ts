...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const result = createMissionPortfolioResolutionManager().requestPortfolioResolutionReview(args);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().then((code) => {
    ...
    process.stdout.write(`${canonicalStringify({ error: 'MISSION_CONTROL_PORTFOLIO_REQUEST_RESOLUTION_REVIEW_UNEXPECTED_RUNTIME_ERROR' })}\n`);
    process.exit(2);
  });
}
