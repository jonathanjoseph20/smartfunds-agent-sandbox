...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const coordination = inspection.inspectCoordination(args.teamId);
    printJson({
      teamId: coordination.teamId,
      priority: coordination.priority,
      readiness: coordination.readiness,
      activeInvestigations: coordination.activeInvestigations,
      stabilizationState: coordination.stabilizationState
    });
    return 0;
  } catch (error) {
