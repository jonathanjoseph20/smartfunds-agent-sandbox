...
export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    ...
    const run = createMissionRunInspection().inspectMissionRun(args);
    printJson({
      missionRunId: run.missionRunId,
      missionId: run.missionId,
      completionState: run.completionState,
      completionPercent: run.progressSummary.completionPercent,
      totalTaskCount: run.progressSummary.totalTaskCount,
      completedTaskCount: run.progressSummary.completedTaskCount,
      failedTaskCount: run.progressSummary.failedTaskCount,
      blockedTaskCount: run.progressSummary.blockedTaskCount,
    });
    return 0;
  } catch (error) {
