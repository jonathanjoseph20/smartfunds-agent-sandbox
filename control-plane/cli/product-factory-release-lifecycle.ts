...
function printJson(value: unknown): void { process.stdout.write(`${canonicalStringify(value)}\n`); }

export async function main(argv: string[] = process.argv.slice(2)): Promise<number> {
  try {
    const args = parseArgs(argv);
    const inspection = createProductFactoryReleaseInspection();
    printJson(inspection.inspectLifecycleAcceptance(args.productFactoryReleaseAcceptanceRecordId));
    return 0;
  } catch (error) {
