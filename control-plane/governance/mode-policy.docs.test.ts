...
function evaluateForFiles(changedFiles: string[], declaredTier: number | null) {
...
describe('mode policy docs neutrality', () => {
  it('passes structured code-only changes (T-M11)', () => {
    const result = evaluateForFiles(['control-plane/validate-pr.ts'], 3);
    expect(result.status).toBe('ok');
  });

  it('passes docs-only changes (T-M12)', () => {
    const result = evaluateForFiles(['docs/runbooks/governance-workflow.md'], 0);
    expect(result.status).toBe('ok');
  });

  it('passes code+docs without mixed-mode failure (T-M13)', () => {
    const result = evaluateForFiles(['control-plane/validate-pr.ts', 'docs/README.md'], 3);
    expect(result.status).toBe('ok');
    expect(result.violation).toBeNull();
  });

  it('preserves autonomous-only behavior for non-doc paths (T-M14)', () => {
    const result = evaluateForFiles(['apps/web/index.tsx'], 0);
    expect(result.status).toBe('ok');
