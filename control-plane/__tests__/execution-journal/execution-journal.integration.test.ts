...
const tmpRoot = path.join('control-plane', '__tests__', 'tmp-execution-journal-integration');
...
describe('execution journal integration', () => {
  it('T-MEJ-I1 full deterministic pipeline stays pre-execution and projection-only', () => {
    ...
    const firstAttempt = executionAttemptInspection.createExecutionAttempt({ runtimeEnvelopeId, attemptIndex: 1 });
    const secondAttempt = executionAttemptInspection.createExecutionAttempt({ runtimeEnvelopeId, attemptIndex: 1 });

    expect(firstAttempt.executionAttemptId).toBe(secondAttempt.executionAttemptId);

    const attemptBeforeJournal = executionAttemptProjection.projectOne({ executionAttemptId: firstAttempt.executionAttemptId });

    const evaluatedFirst = journalInspection.evaluateExecutionJournal({ executionAttemptId: firstAttempt.executionAttemptId });
    const evaluatedSecond = journalInspection.evaluateExecutionJournal({ executionAttemptId: firstAttempt.executionAttemptId });

    expect(evaluatedSecond).toEqual(evaluatedFirst);

    const materializedFirst = journalInspection.materializeExecutionJournal({ executionAttemptId: firstAttempt.executionAttemptId });
    const materializedSecond = journalInspection.materializeExecutionJournal({ executionAttemptId: firstAttempt.executionAttemptId });
    ...
    const firstSnapshot = {
      ...
      events: fs.readFileSync(materializedFirst.eventsPath, 'utf8'),
    };
    ...
    const secondSnapshot = {
      ...
      events: fs.readFileSync(materializedSecond.eventsPath, 'utf8'),
    };

    expect(secondSnapshot).toEqual(firstSnapshot);

    const attemptAfterJournal = executionAttemptProjection.projectOne({ executionAttemptId: firstAttempt.executionAttemptId });
    expect(attemptAfterJournal).toEqual(attemptBeforeJournal);
